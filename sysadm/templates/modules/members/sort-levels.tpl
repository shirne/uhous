...
{{ webcontrol type='Position' name='position' column='栏目管理' position='会员等级排序' footprint='yes' }}
...
<script type="text/javascript" src="/sysadm/js/sortselect.js"></script>
    ...
    <div class="layout clearfix mt">
        ...
        <form id="sortform" name="sortform" action="{{ url controller='Member' action='SaveSortLevels' }}" method="post">
            ...
            <div class="layout-left">
                ...
                <div class="box">
                    <h3>等级</h3>
                    <div class="form">
                        <select id="sort" name="sort" size="26" style="width:100%" ondblclick="window.location.href='{{ url controller="Member" action="SortLevels" }}&parent_id=' + $('#sort option:selected').val()">
{{ section name=category loop=$topCategories }}
                            <option value="{{$topCategories[category].level_id}}">{{ $smarty.section.category.iteration }}.{{$topCategories[category].levels}}</option>
{{ /section }}
                        </select>
                    </div>
                </div>
