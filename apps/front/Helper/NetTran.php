 ...
 * @date 2008.5.15
 */
DEFINE("OPEN_SSL_CONF_PATH", dirname(__FILE__)."/../cer/openssl.cnf");//point to your config file
...
/**
	 ...
	 * @param EndTime 结束时间(查询BeginTime到EndTime的交易结果数据，格式为：yyyy-mm-dd hh:mm:ss)
         * @param OrderNo 订单号
	 * @return 下载成功返回true(从LastResult属性获取结果)，失败返回false(从LastErrMsg属性获取失败原因)
	 */
	 function GetResult($MerId,$UserId,$Pwd,$PaySuc,$ShoppingTime,$BeginTime,$EndTime,$OrderNo)
	{
		//从配置文件中读出目标页面URL
		$configFile="OpenVendor.ini";
